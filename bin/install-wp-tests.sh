...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+\-(beta|RC)[0-9]+$ ]]; then
	...
	# The version-check endpoint can return a bare major.minor (e.g., 7.1)
	# even though the wordpress-develop repo always tags the full x.y.z (e.g., 7.1.0).
...
install_woocommerce() {
	...
	unzip -q -o "$TMPDIR/woocommerce.zip" -d "$TMPDIR/"
	rm -rf "$PLUGIN_DIR/woocommerce"
	mv "$TMPDIR/woocommerce" "$PLUGIN_DIR/woocommerce"
	echo -e "${GREEN}WooCommerce plugin installed successfully.${RESET}"
}

install_theme() {
	local THEMES_DIR="$WP_CORE_DIR/wp-content/themes"
	mkdir -p "$THEMES_DIR"
	rm -rf "$THEMES_DIR/close-masaifood-theme"
	ln -s "$THEME_DIR" "$THEMES_DIR/close-masaifood-theme"
	echo -e "${GREEN}Theme symlinked into test install successfully.${RESET}"
}
