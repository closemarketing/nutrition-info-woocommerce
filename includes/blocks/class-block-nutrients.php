...
class NIW_Block_Nutrients {
	...
	public function rest_render( $request ) {
		$opts = array(
			...
			'group_bg'     => $request->get_param( 'group_bg' ) ? $request->get_param( 'group_bg' ) : '#f0f0f0',
			'group_text'   => $request->get_param( 'group_text' ) ? $request->get_param( 'group_text' ) : '#555555',
			...
			'sub_text'     => $request->get_param( 'sub_text' ) ? $request->get_param( 'sub_text' ) : '#666666',
			'border_color' => $request->get_param( 'border_color' ) ? $request->get_param( 'border_color' ) : '#e8e8e8',
			'font_size'    => absint( $request->get_param( 'font_size' ) ) ? absint( $request->get_param( 'font_size' ) ) : 13,
		);
	...
	public function render( $attributes ) {
		...
		$opts          = array(
			...
			'font_size'    => $font_size_raw ? $font_size_raw : 13,
		);
		return $this->build_html( $product_id, $opts );
	}

	/**
	 * Build nutritional info HTML table.
	 *
